...
typedef struct {
	Int		file_nr;	/* File number, refer to thread_access_t */
	UInt	line1;		/* Line number for first thread */
	UInt	line2;		/* Line number for second thread */
} collision_t;
...
static Bool insert_last_exclusive_malloc_collision(vector_t* vector, collision_t* collision)
{
	...
	col->file_nr = collision->file_nr;
	col->line1 = collision->line1;
	col->line2 = collision->line2;
	...
	for (i = 1; i < size(g_accesses); i++) {
		...
		for (j = 0; j < elements; j++) {
			...
			added = insert_last_exclusive_malloc_collision(g_results,
				& (collision_t) {	.file_nr = access->file_nr,
									.line1 = access->line,
									.line2 = other_access->line
								});
			if (added)
				VG_(printf)("false sharing possible in \"%s\" with lines %d vs %d\n", fg_get_name_from_id(access->file_nr), access->line, other_access->line);			
		}
	}
}
